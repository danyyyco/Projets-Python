...
def lancer_jeu(nb_essais_max, plafond, mode_nom, temps_limite=60):
    ...
    while nb_essais <= nb_essais_max:
        ...
        """try:
            nb_utilisateur = int(input(Fore.YELLOW + f"👉 {prenom_utilisateur}, devine le nombre : " + Style.RESET_ALL))
        except ValueError:
        ...
        if nb_utilisateur == r:
            print(Fore.GREEN + f"\n🎉 Bravo {prenom_utilisateur} ! Tu as trouvé le nombre {r} en {nb_essais} essais et {int(temps_ecoule)}s ! 🏆")
            score += 1
            nb_victoires += 1
            break
...
if choix_continuer.lower() == "n":
    print(Fore.GREEN + f"\n👋 Merci d'avoir joué {prenom_utilisateur} ! Reviens vite pour plus de défis 🤩 !")